...
class DoBotArm:
  ...
  def toggleSuction(self, wait=True):
    lastIndex = 0
    if (self.suction):
      self.lastIndex = dType.SetEndEffectorSuctionCup(
  ...
  def moveArmRelXYZ(self, xrel, yrel, zrel, wait=True, jump=False):
    positions = self.getPosition()
    return self.moveArmXYZ(positions[0] + xrel, positions[1] + yrel,
                           positions[2] + zrel, wait, jump)
  ...
  def moveHome(self, wait=True):
    self.lastIndex = dType.SetPTPCmd(self.api, dType.PTPMode.PTPMOVLXYZMode,
                                     self.homeX, self.homeY, self.homeZ,
                                     self.rotation)[0]
    if (wait):
      self.commandDelay(self.lastIndex)
    return self.lastIndex
